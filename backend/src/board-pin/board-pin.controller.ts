...
export class BoardPinController {
	...
	@ApiOperation({
		...
		description:
			'Authenticated users can save a pin to a board, ' +
	...
	@Post()
	async create(@UserId() userId: string, @Body() createBoardPinDto: CreateBoardPinDto): Promise<BoardPinDocument> {
	...
	@Post('merge')
	merge(@UserId() userId: string, @Body() mergeBoardDto: MergeBoardDto) {
		return this.boardPinService.mergeBoard(mergeBoardDto, userId)
	}
}
